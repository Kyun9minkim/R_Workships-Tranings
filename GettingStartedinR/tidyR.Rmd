...
These guidelines may look familiar now, as similar ideas form the basis of good database design. The tables in our sample dataset (*surveys*, *species*) are already in a tidy format. Let's consider a different examples where the counts of three species are recorded for each day in a week:
```{r}
...
Let's load the **tidyr** package and use its `pivot_longer` function to reshape *counts_df* into a tidy format (remember to first install the tidyr package if you haven't before):
```{r}
library(tidyverse)
...
counts_df
```
One of the neat aspects of tidyr and dplyr is that each function takes a data frame as its first parameter and returns the transformed data frame. As we will see later, it makes it very easy to apply these functions in a chain. Here, `pivot_longer` takes the column names and values except for the values of the days column and reshapes them into two columns, the names of which are specified as the names_to and values_to The names_to column contains the column name corresponding to each value in the original dataset.

The inverse of `pivot_longer` is `pivot_wider`. You can confirm that the code below restores the original form of *counts_df*:
```{r}
...
head(species) #look at headers and first few rows of species
```
...
Before moving down, try to write the code to create a new date column (YYYY/MM/DD format) in the *surveys* data frame while keeping all the existing columns. 
...
To sample a certain number or a certain fraction of rows at random, use `sample_n` and `sample_frac`, respectively. [make sure you have installed the dplyr package!]
```{r message = FALSE}
library(dplyr)
...
`arrange` sorts rows over one or multiple columns. We'll use arrange to sort the randomly sampled surveys (surveys_pick100) first by species and then by weight (wgt) in descending order (desc). As a comparison, the second sort is using base R code doing the same operation so you can see the ease of the function.  
```{r}
sorted1 <- arrange(surveys_pick100, species, desc(wgt)) #with dplyr
sorted2 <- surveys_pick100[order(surveys_pick100$species,  -surveys_pick100$wgt),] #in baseR
head(sorted1)
head(sorted2)
```

To select rows based on certain conditions (like WHERE in SQL), use `filter`. Note that a logical "and" is implied when conditions are separated by commas. So the next code is a selection for surveys where the year is 1990 and the months are janurary-march.
```{r}
...
```{r}
head(surveys1990_winter) #first view the columns before we remove a column
surveys1990_winter <- select(surveys1990_winter, -year) #remove year column
head(surveys1990_winter) #view the new data frame
```
...
Looking at the whole *counts_1990w_join* data frame, you may notice the last row of *counts_1990w* (where the species info. was *NA*) was excluded. An `inner_join` only keeps rows for which a match was found in the other table. To keep all rows from the first table, we could use `left_join` instead.
```{r eval = FALSE}
counts_1990w_join <- left_join(counts_1990w, species, 
                               by = c("species" = "species_id"))
```

The last dplyr key function is `mutate`, which creates new columns by performing the same operation on existing values in each row. Here we use the absolute counts from *counts_1990w_join* to calculate the proportion of each species.

After you finish the last line of code send your code for Exercises A and B to Professor Garcia with the subject "EVE 314 - answers"
```{r}
...
[Data wrangling with dplyr and tidyr (RStudio cheat sheet)](https://github.com/rstudio/cheatsheets/blob/main/tidyr.pdf)
